...
bool c_player::Update(float delta)
{
	...
	if (hge->Input_GetKeyState(HGEK_LEFT) && Position.x > 32) Velocity.x -= (Speed * M_PI) / 2;
	if (hge->Input_GetKeyState(HGEK_RIGHT) && Position.x < 800) Velocity.x += (Speed * M_PI) / 2;
	if (hge->Input_GetKeyState(HGEK_UP) && Position.y > 0) Velocity.y -= (Speed * M_PI) / 2;
	if (hge->Input_GetKeyState(HGEK_DOWN) && Position.y < 600) Velocity.y += (Speed * M_PI) / 2;

	if (Position.x > 300) { Position.x -= 1; Velocity.x = -Velocity.x; };
	if (Position.x < 50)   { Position.x += 1; Velocity.x = -Velocity.x; };
	if (Position.y < 50) { Position.y += 1; Velocity.y = -Velocity.y; };
	if (Position.y > 750) { Position.y -= 1; Velocity.y = -Velocity.y; };
